...
class TicketService:
    ...
    def watchers(self, ticket_id: int) -> list[User]:
        """Ejercicio 2: devuelve el solicitante y, si existe, el técnico
        asignado, sin repetir usuarios con el mismo id. Se apoya únicamente
        en self.require (ticket) y self._users.require (usuarios), sin
        acceder a repositorios ajenos directamente.
        """
        ticket = self.require(ticket_id)
        observers: list[User] = [self._users.require(ticket.requester_id)]

        if ticket.assignee_id is not None:
            technician = self._users.require(ticket.assignee_id)
            already_present = any(user.id == technician.id for user in observers)
            if not already_present:
                observers.append(technician)
    ...
    def assign(
        ...
        # Ejercicio 3: rechazar la reasignación al mismo técnico antes de
        # tocar historial o notificaciones, y antes de cualquier otra
